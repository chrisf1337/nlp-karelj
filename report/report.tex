...
begin with---and the (intuitively-named) function \texttt{sum()} performs the summing action.
...
phase, we convert the actions and conditionals into Java code and inject it into a template file
...
language Karel commands can be expressed in Karel Java code.
...
contains a full text description of the state of the world at the start and end of each test case
---the size of the world; the positions and numbers of beepers at grid coordinates; the position, orientation, and number of beepers held by the robot; and the position and direction of walls.
